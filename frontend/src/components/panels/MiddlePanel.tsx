...
export default function MiddlePanel() {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (imageId && images.find(img => img.id === imageId)) {
      // Use the existing toggleSelect function with no modifier keys for single selection
  ...
  return (
    <div 
      className={`h-full w-full flex flex-col items-center justify-center gap-6 p-6 overflow-auto transition-all duration-200 ${
        isDragOver ? 'bg-[var(--selection)] border-2 border-dashed border-[var(--accent-blue)]' : ''
      }`}
      onDragOver={handleDragOver}
      onDragLeave={handleDragLeave}
    ...
    >
      ...
      <div className="w-full h-[60vh] rounded-2xl shadow-2xl overflow-hidden border border-[var(--border)] bg-[var(--bg-secondary)]">
        <ImageViewer 
          ref={imageViewerRef}
          imageUrl={image.url} 
          alt={image.filename}
